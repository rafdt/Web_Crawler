...
class Frontier:
    ...
    def __init__(self):
        ...
        self.urls_set = set()
        self.fetched = 0
        self.domains = dict() #key: subdomain, value: count of URLS processed from respective subdomain
    ...
    def add_url(self, url):
        ...
        if not self.is_duplicate(url):
            ...
            #keeps track of subdomain visits with a count of how many different URLS it's processed
            if (parsed.netloc not in self.domains):
                self.domains[parsed.netloc] = 1
            else:
                self.domains[parsed.netloc] += 1
